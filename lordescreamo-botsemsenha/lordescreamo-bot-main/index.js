...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:CarlosX\n'
            + 'ORG:Loser Bot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=8499034946:+1 (514) 416-7859\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '.'
blocked = []
limitawal = '50'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*SEU LEVEL AINDA ESTA FRACO* 0 °-°',
				error: {
			...
			const ownerNumber = ["8499034946@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'LOSER_BOT'
                        const ownerName = 'CARLOS_NAME'
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   break
				case 'ytmp3':
					if (args.length < 1) return reply('Insira o link/url')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Porfavor insira o link/url do video.')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Marcando Geral 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mercando 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Adeus, Obrigado por participar 👋', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('onde esta o titulo da musica kak')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Qual o nome da musica mana?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('O que voce deseja adicionar??')
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, vamos remover os envolvidos:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'ocr':
                    ...
                    reply('Seu reporte foi enviado para o proprietario do bot. Obrigado Pela sua ajuda.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha, erro na hora da conversao, verifique e tente novamente ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}lovepaper screamoCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
